...
def write_annotation(doc):
    ...
    add_paragraph(doc, (
        ...
        "з поступовим ускладненням завдань."
    ))

    add_paragraph(doc, (
        "Наукова новизна полягає у поєднанні monorepo-архітектури як навчального "
        "інструменту зі спринтовою методологією, що створює "
...
def write_toc(doc):
    """Write ЗМІСТ page."""
    add_page_break(doc)
    add_chapter_title(doc, "ЗМІСТ")

    toc_items = [
        ("Вступ", "7"),
        ("Розділ 1. Аналіз предметної області", "10"),
        ("1.1. Проблема набуття практичних навичок full-stack розробки", "10"),
        ("1.2. Огляд існуючих навчальних платформ та їх обмеження", "13"),
        ("1.3. Monorepo-архітектура: концепція та переваги", "17"),
        ("1.4. Обґрунтування вибору технологій та інструментів", "19"),
        ("1.5. Висновки за розділом", "21"),
        ("Розділ 2. Проєктування навчальної платформи", "22"),
        ("2.1. Концепція платформи та навчальна модель", "22"),
        ("2.2. Сценарії використання платформи", "24"),
        ("2.3. Архітектура Monorepo-системи", "26"),
        ("2.4. Проєктування спільних пакетів", "28"),
        ("2.5. Проєктування бази даних", "30"),
        ("2.6. Проєктування REST API та спринтовий роутинг", "31"),
        ("2.7. Проєктування web-додатку", "33"),
        ("2.8. Проєктування mobile-додатку", "35"),
        ("2.9. Система навчальних завдань (тікети та спринти)", "36"),
        ("2.10. Використання AI-асистента в навчальному процесі", "38"),
        ("2.11. Висновки за розділом", "40"),
        ("Розділ 3. Реалізація програмного продукту", "41"),
        ("3.1. Реалізація серверної частини", "41"),
        ("3.2. Реалізація спільних пакетів", "44"),
        ("3.3. Реалізація web-додатку", "45"),
        ("3.4. Реалізація mobile-додатку", "47"),
        ("3.5. Реалізація AI-асистента", "48"),
        ("3.6. Інтеграція компонентів та демонстрація роботи платформи", "50"),
        ("3.7. Висновки за розділом", "51"),
        ("Розділ 4. Тестування та оцінювання ефективності", "52"),
        ("4.1. Методика тестування", "52"),
        ("4.2. Функціональне тестування серверного додатку", "53"),
        ("4.3. Інтеграційне тестування", "55"),
        ("4.4. Тестування спільних пакетів", "56"),
        ("4.5. Оцінювання ефективності навчального процесу", "57"),
        ("4.6. Висновки за розділом", "58"),
        ("Загальні висновки", "59"),
        ("Список використаних джерел", "61"),
        ("Додаток А. Конфігурація Monorepo", "63"),
        ("Додаток Б. Код спільних пакетів", "65"),
        ("Додаток В. Приклад навчального тікету", "68"),
    ]
    ...
    for title, page in toc_items:
        ...
        # Indent subsections
        if title[0].isdigit() and '.' in title[:4]:
            para.paragraph_format.left_indent = Cm(1.0)

        # Add tab stop at right margin for page numbers
...
def write_introduction(doc):
    ...
    add_paragraph(doc, (
        ...
        "проєктом та поспринтову систему."
    ))
    ...
    add_paragraph(doc, (
        ...
        "навчальної платформи, що представляє monorepo-архітектуру як навчальний "
        "інструмент зі спринтовою методологією поступового ускладнення завдань. "
        "На відміну від існуючих рішень, платформа надає "
    ...
    add_paragraph(doc, (
        ...
        "Платформа може бути використана у "
...
def write_chapter1(doc):
    ...
    add_paragraph(doc, (
        ...
        "Перевагами є структурований навчальний план та те, що вона є безкоштовною. "
    ...
    add_paragraph(doc, (
        ...
        "втрачають свою актуальність через швидкий розвиток технологій. Студент "
    ...
    conclusions = [
        ...
        "Проаналізовано існуючі навчальні платформи та виявлено їх обмеження: "
        "ізольованість завдань, відсутність повноцінного full-stack досвіду та спринтової моделі.",
...
def write_chapter2(doc):
    ...
    add_paragraph(doc, (
        ...
        "AI-асистента в розумній IDE або переглянути референсну реалізацію "
        "frontend-додатку у monorepo."
    ))
    ...
    add_paragraph(doc, (
        "На будь-якому етапі роботи студент може використовувати вбудованого "
        "AI-асистента в розумній IDE. Асистент має доступ до контексту проєкту: "
        "структури monorepo, технологічного стеку, змісту тікетів поточного "
        "спринту та прийнятих архітектурних рішень. Наприклад, якщо студент "
        "запитає: «Як реалізувати захищений маршрут у React?», асистент пояснить "
        "відповідні концепції, запропонує можливі підходи до реалізації та "
        "зверне увагу на важливі аспекти, але не надаватиме готового коду. "
        "Такий формат роботи відображає сучасну практику використання "
        "AI-інструментів розробниками під час створення програмного забезпечення."
    ))
    ...
    # 2.10
    add_subsection_title(doc, "2.10. Використання AI-асистента в навчальному процесі")

    add_paragraph(doc, (
        "Для підтримки студентів під час виконання практичних завдань "
        "використовується AI-асистент, інтегрований у сучасне середовище "
        "розробки (IDE). Асистент реалізований у вигляді чату на основі "
        "великої мовної моделі (LLM), який має доступ до відкритої кодової "
        "бази проєкту та може аналізувати її структуру."
    ))

    add_paragraph(doc, (
        "На відміну від звичайних чат-ботів, AI-асистент працює безпосередньо "
        "в контексті проєкту: він бачить структуру monorepo, технологічний "
        "стек, файли, модулі та поточні зміни в коді. Завдяки цьому "
        "відповіді формуються з урахуванням конкретного навчального проєкту."
    ))

    add_paragraph(doc, "Робота асистента ґрунтується на таких компонентах:")
    add_list_item(doc, "Контекст проєкту — структура репозиторію, технологічний стек, поточний спринт, зміст тікетів та кодова база;")
    add_list_item(doc, "Системні інструкції — визначають роль асистента як ментора, а не виконавця, та встановлюють обмеження щодо надання готових рішень;")
    add_list_item(doc, "База знань — документація проєкту, архітектурні рішення, типові помилки та рекомендації щодо їх усунення.")

    add_paragraph(doc, "Основні сценарії використання AI-асистента:")
    add_list_item(doc, "пояснення теоретичних понять у контексті проєкту, наприклад запит «Що таке JWT?»;")
    add_list_item(doc, "надання рекомендацій щодо реалізації функціональності, наприклад запит «Як реалізувати захищений маршрут?»;")
    add_list_item(doc, "допомога в пошуку причин помилок та аналізі повідомлень про помилки;")
    add_list_item(doc, "навігація по кодовій базі та структурі проєкту, наприклад запит «Де знаходиться конфігурація API?»;")
    add_list_item(doc, "пояснення взаємодії між модулями та компонентами системи.")

    add_paragraph(doc, (
        "Ключовий принцип використання AI-асистента полягає в тому, що він "
        "виконує роль наставника. Асистент допомагає студенту зрозуміти "
        "концепції, пропонує напрямки пошуку рішення та пояснює архітектурні "
        "особливості проєкту, але не замінює самостійне виконання завдань. "
        "Такий підхід сприяє розвитку практичних навичок розробки та "
        "формуванню здатності самостійно вирішувати професійні задачі."
    ))
    ...
    conclusions = [
        ...
        "Передбачено підключення AI-асистента для покращення навчального процесу на основі розумних IDE.",
    ]
    for i, c in enumerate(conclusions, 1):
        add_numbered_item(doc, i, c)
...
def write_chapter3(doc):
    ...
    add_page_break(doc)
    add_chapter_title(doc, "РОЗДІЛ 3. РЕАЛІЗАЦІЯ ПРОГРАМНОГО ПРОДУКТУ")
  ...
  switch (sprint) {
    ...
    add_paragraph(doc, (
        "AI-асистент можна використовувати як готовий допоміжний інструмент, "
        "що працює в дуеті з навчальною кодовою базою. Він вбудований у "
        "платформу у вигляді опціонального чату (на кшталт інтерфейсу в "
        "Antigravity). Асистент не є частиною коду, який студенти мають "
        "розробляти чи конфігурувати, — це зовнішній віртуальний ментор, "
        "який підключається до вашого проєкту «ззовні», допомагає в ньому "
        "орієнтуватися та викликається суто за бажанням користувача."
    ))

    add_paragraph(doc, (
        "Синхронізація асистента з кодовою базою проєкту відбувається "
        "завдяки системному промпту. Платформа автоматично витягує "
        "актуальний контекст розробленого вами репозиторію та передає "
        "його у велику мовну модель (LLM). Цей контекст включає:"
    ))
    add_paragraph(doc, "Структуру monorepo: архітектуру вашої кодової бази та чітке призначення кожного пакету;")
    add_paragraph(doc, "Технологічний стек: перелік та версії бібліотек, на яких побудовано ваш проєкт;")
    add_paragraph(doc, "Зміст спринтів: опис тікетів і завдань, які студент має виконати в межах цієї кодової бази;")
    add_paragraph(doc, "Архітектурні рішення: патерни та підходи, закладені вами у фундамент проєкту;")
    add_paragraph(doc, "Базу знань ментора: типові помилки, яких припускаються студенти під час роботи з вашим кодом, та методи їх виправлення.")

    add_paragraph(doc, (
        "Системний промпт регулює поведінку штучного інтелекту під час "
        "взаємодії з кодом. Він виступає в ролі досвідченого ментора до "
        "вашого проєкту: пояснює логіку написаних вами модулів, дає підказки "
        "щодо архітектури та направляє студента, але принципово не генерує "
        "готовий код для копіювання, змушуючи працювати самостійно."
    ))

    add_paragraph(doc, (
        "Асистент адаптує рівень комунікації залежно від етапу роботи з "
        "кодовою базою. Наприклад, для Sprint 1 (знайомство з вашим проєктом) "
        "пояснення архітектури є максимально детальними, а для Sprint 4 — "
        "лаконічними та технічно концентрованими, оскільки студент уже має "
        "орієнтуватися в коді."
    ))

    add_paragraph(doc, (
        "З технічного погляду, платформа надає підготовлений контекст вашої "
        "кодової бази у форматі Markdown-файлів. Це забезпечує повну свободу "
        "вибору інструментів взаємодії:"
    ))
    add_paragraph(doc, (
        "Вбудований чат: Використання стандартного візуального віджета "
        "безпосередньо на навчальній платформі (через OpenAI API, Anthropic "
        "Claude або локальні моделі типу Ollama)."
    ))
    add_paragraph(doc, (
        "IDE-інтеграції (Cursor, GitHub Copilot, Antigravity): Якщо студенту "
        "зручніше працювати в дуеті з асистентом безпосередньо у редакторі "
        "коду, де відкрита ваша навчальна база, він може просто завантажити "
        "ці Markdown-файли контексту у свій робочий простір."
    ))
...
def write_conclusions(doc):
    ...
    conclusions = [
        ...
        "та виявлено їх ключові обмеження: ізольованість завдань, відсутність досвіду "
